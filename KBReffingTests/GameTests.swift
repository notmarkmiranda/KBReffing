...
class GameTests: XCTestCase {
    func game(status: GameStatus = GameStatus.notStarted) -> Game {
    ...
    func testFormattedDate() throws {
        XCTAssertEqual(game().formattedDate(), "May 9, 2015")
    }
    
    func testStringStatusNotStarted() throws {
        XCTAssertEqual(game().stringStatus(), "Not Started")
    }
    
    func testStringStatusInProgress() throws {
        let status = GameStatus.inProgress
        XCTAssertEqual(game(status: status).stringStatus(), "In Progress")
    }
    
    func testStringStatusFinished() throws {
        let status = GameStatus.finished
        XCTAssertEqual(game(status: status).stringStatus(), "Finished")
    }
}
